...
export function ProfileCard({ person, childrenList }: { person: TaromboPerson | null; childrenList: TaromboPerson[] }) {
    if (!person) {
        return (
            <div className="flex w-full max-w-sm flex-col items-center justify-center rounded-2xl border border-tb-outline-variant bg-tb-surface-bright p-6 text-center shadow-xl shadow-tb-surface-container-high/30">
                <p className="text-sm text-tb-on-surface-variant">Pilih anggota pada diagram untuk melihat detail.</p>
            </div>
        );
    }
    ...
    const detailRows = [
        ...
        { label: 'Generasi', value: `Gen ${person.generation} (${getGenerationLabel(person.generation)})` },
    ];

    const childNames =
        person.childrenNames ??
        childrenList.map((child) => (child.birthYear ? `${child.name} (${child.birthYear})` : child.name));
    ...
    return (
        <div className="relative w-full max-w-sm rounded-2xl border border-tb-outline-variant bg-tb-surface-bright p-6 shadow-xl shadow-tb-surface-container-high/30">
            ...
            <div className="mb-6 text-center">
                <p className="text-xs font-semibold uppercase tracking-wider text-tb-on-surface-variant">
            ...
            <div className="space-y-4 text-sm">
                {detailRows.map((row) => (
                    <div key={row.label}>
                        <p className="mb-1 text-xs font-semibold text-tb-on-surface-variant">{row.label}</p>
                        <p>{row.value}</p>
                    </div>
                ))}
                <div>
                    <p className="mb-1 text-xs font-semibold text-tb-on-surface-variant">
                        Anak {childNames.length > 0 ? `(${childNames.length})` : ''}
                    </p>
                    {childNames.length > 0 ? (
                        ...
                        <p className="text-tb-on-surface-variant">Belum ada data anak.</p>
                    )}
                </div>
                <div>
                    <p className="mb-1 text-xs font-semibold text-tb-on-surface-variant">Bio</p>
            ...
            <button
                type="button"
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-tb-surface-container py-2.5 text-sm font-medium transition-colors hover:bg-tb-surface-container-high"
            >
